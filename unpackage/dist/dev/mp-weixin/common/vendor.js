...
module.exports = {
  // 页面跳转
  push: function push(path) {var _this = this;var option = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    if (typeof path == 'string') {
      option.path = path;
    } else {
      option = path;
    }
    var url = routerPath[option.path];var _option =


    option,_option$query = _option.query,query = _option$query === void 0 ? {} : _option$query,openType = _option.openType,duration = _option.duration;
    var params = this.parse(query);
    if (params) {
      url += '?' + params;
    }
    duration ? setTimeout(function () {
      _this.to(openType, url);
    }, duration) : this.to(openType, url);
  },
  to: function to(openType, url) {
    var obj = {
      url: url };
...
function _classCallCheck(instance, Constructor) {if (!(instance instanceof Constructor)) {throw new TypeError("Cannot call a class as a function");}}var Config =
function Config() {"use strict";_classCallCheck(this, Config);
